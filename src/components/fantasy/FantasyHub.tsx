  ...
  return (
    <div className="min-h-screen bg-[#000000]">
        ...
        <div className="flex items-start justify-between mb-3">
          <div>
            <div className="flex items-center gap-1.5 mb-1">
              ...
              <span className="text-[#FF4444] font-black text-[11px] tracking-wide">CIERRE JORNADA 1</span>
            </div>
            <p className="text-gray-600 text-[11px]">Arma tu XI y predice alineaciones</p>
          </div>
          <div className="flex items-start gap-1.5">
            <div className="text-center">
              <div className="text-white font-black text-xl leading-none">{String(ui.timeH).padStart(2, '0')}</div>
              <div className="text-gray-600 text-[10px] font-bold mt-0.5">HRS</div>
            </div>
            <div className="text-gray-600 font-black text-base pt-0.5">,</div>
            <div className="text-center">
              <div className="text-white font-black text-xl leading-none">{String(ui.timeM).padStart(2, '0')}</div>
              <div className="text-gray-600 text-[10px] font-bold mt-0.5">MIN</div>
            </div>
            <div className="text-gray-600 font-black text-base pt-0.5">,</div>
            <div className="text-center">
              <div className="text-white font-black text-xl leading-none">{String(ui.timeS).padStart(2, '0')}</div>
              <div className="text-gray-600 text-[10px] font-bold mt-0.5">SEG</div>
            </div>
          </div>
        </div>
        <div className="flex items-center justify-between">
          <div className="text-center">
            <div className="text-white font-black text-lg">{FMT(ui.online)}</div>
            <div className="text-gray-600 text-[10px]">managers online</div>
          </div>
          <div className="text-center">
            <div className="text-[#00E676] font-black text-lg">{FMT(ui.lined)}</div>
            <div className="text-gray-600 text-[10px]">ya armaron XI</div>
          </div>
          <div className="text-center">
            <div className="font-black text-lg">🔥 <span className="text-white">{state.streak}</span></div>
      ...
      <div className="flex gap-2 px-4 py-3 overflow-x-auto bg-[#000]">
        <button className="bg-[#00E676] text-black font-bold text-[11px] py-2 px-3 rounded-lg whitespace-nowrap flex items-center gap-1">
          🏠 Hub
        </button>
        <button
      ...
      <div className="px-4 py-3 bg-[#000]">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-white font-bold text-sm flex items-center gap-2">⚽ Mi Selección</h2>
          <div className={`text-[11px] font-black px-2 py-0.5 rounded ${
            state.squad.length >= 11
              ? 'text-[#00E676] bg-transparent'
        ...
        <div className="flex gap-1 flex-wrap mb-2">
          {state.squad.slice(0, 11).map(p => (
            <div
              ...
              className={`w-8 h-6 rounded-sm overflow-hidden ${
                state.captainId === p.id ? 'ring-2 ring-[#FFD700]' : 'border border-[#333]'
              }`}
            >
              <img
          ...
          {Array(Math.max(0, 11 - state.squad.length)).fill(0).map((_, i) => (
            <div key={`empty-${i}`} className="w-8 h-6 rounded-sm border border-dashed border-[#333]" />
          ))}
        </div>
        <div className="flex justify-between text-[11px]">
          <span className="text-gray-600">
            Presupuesto: <span className="text-[#00E676] font-black">${state.budget}M</span>
            <span className="mx-1">·</span>
            Formación: <span className="text-gray-400 font-bold">{state.formation}</span>
          </span>
          <span className="text-[#00E676] font-bold cursor-pointer" onClick={() => onNavigate(squadOk ? 'pitch' : 'squad')}>
            Gestionar →
          </span>
        </div>
      </div>
      ...
      <div className="px-4 py-3 bg-[#000] border-t border-[#1a1a1a]">
        <div className="flex items-center justify-between mb-2">
          <div>
            <h2 className="text-white font-bold text-sm flex items-center gap-2">🎯 Predice Alineaciones</h2>
            <p className="text-gray-600 text-[10px]">+2 pts por titular acertado · +5 pts si aciertas esquema</p>
          </div>
          <div className="text-[#00E676] text-xs font-bold">{totalPred}/{totalMatch}</div>
        </div>
        ...
        {md.matches.length > 4 && (
          <div className="text-center py-2 text-xs text-[#00E676] font-bold cursor-pointer">
      ...
      <div className="grid grid-cols-3 gap-3 px-4 py-4 bg-[#000]">
        {[
          ...
          >
            ...
            <div className="text-xs font-black text-white mb-0.5">{a.label}</div>
      ...
      <div className="px-4 py-4 bg-[#000] border-t border-[#1a1a1a]">
        <h3 className="text-[#FFD700] font-bold text-sm mb-3 flex items-center gap-2">💡 ¿Cómo funciona?</h3>
        <div className="text-xs text-gray-500 leading-relaxed space-y-1.5">
          <div><span className="text-gray-400 font-bold">1.</span> Arma tu selección de 15 jugadores con presupuesto de $250M</div>
          <div><span className="text-gray-400 font-bold">2.</span> Define tus 11 titulares, suplentes, capitán y formación</div>
          <div><span className="text-gray-400 font-bold">3.</span> Predice las alineaciones titulares de los equipos reales (+2 pts/acierto)</div>
          <div><span className="text-gray-400 font-bold">4.</span> Tus jugadores suman puntos por goles, asistencias, porterías a 0</div>
          <div><span className="text-gray-400 font-bold">5.</span> Si un equipo es <span className="text-[#FF4444]">eliminado</span>, reemplaza esos jugadores</div>
          <div><span className="text-gray-400 font-bold">6.</span> Usa comodines estratégicamente — son limitados todo el torneo</div>
        </div>
      </div>
    </div>
  );
