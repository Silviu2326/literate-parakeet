...
export function usePredictions() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      exact: predictions.filter(p => p.result === 'EXACT').length,
      winner: predictions.filter(p => p.result === 'WINNER').length,
...
export function useChips() {
  ...
  const activeChips = useMemo(() => {
    return myChips.filter(c => c.isActive);
  }, [myChips]);
  
  const availableChips = useMemo(() => {
    return myChips.filter(c => c.usesRemaining > 0 && !c.isActive);
  }, [myChips]);
  
  const usedChips = useMemo(() => {
    return myChips.filter(c => c.totalUses - c.usesRemaining > 0);
  }, [myChips]);
  ...
  return {
    ...
    availableChips,
    usedChips,
    hasActiveChip: activeChips.length > 0,
    activateChip,
    deactivateChip,
  };
}
